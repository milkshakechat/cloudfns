...
import { initQuantumLedger_Drivers, permaTransfer_QuantumLedger } from '../../services/ledger';
...
export const permaTransfer = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    try {
        ...
        if (!checkIfTradingWallet(body.senderWallet) || !checkIfTradingWallet(body.receiverWallet)) {
            return {
                statusCode: 400,
                body: JSON.stringify({
                    message: 'Only trading wallets can send or receive perma transfers',
                }),
            };
        }
        console.log('body', body);
        const transaction = await permaTransfer_QuantumLedger(body);
